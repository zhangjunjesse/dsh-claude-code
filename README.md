...
- 内置 **Claude Code 监控面板**：会话头「对话 / 轨迹」右边多一个 **Claude Code** 标签页，紧凑单行额度栏（点开看 5h/7d 进度条 + 模型专项）+ 横向滚动的任务 tab 条（`ⓘ` 看完整任务 / 模型 / 统计）+ 原生风格实时输出（Markdown 直接预览 / 工具卡片 / 可折叠思考）+ 一键取消（Web 端）。
...
- **顶部额度栏（紧凑单行）**：默认只占一行 —— 订阅档位（如 `Claude Max · 20x`）+ `5h 2% · 7d 3%` 百分比 + 正常 / 注意 / 已阻塞状态徽标 + 刷新按钮；点这一行展开，才显示 5 小时 / 7 天两条进度条（重置时间，≥50% 转黄、≥80% 转红）、模型专项徽标（如 `Fable 0%`）和缓存年龄。数据来自 claude CLI 自己的本地缓存（和 `claude_code_usage` 工具同源），刷新只是重读缓存、不会消耗额度；挂载时读一次，之后每 5 分钟一次。
- **上方任务 tab 条**：本会话的全部 `claude-code` 委派，一个任务一个 tab（状态色点 —— 运行中蓝色呼吸、已完成绿、已失败红、已取消灰 —— 加单行截断标题）；运行中在前（按开始时间），已结束按新到旧；任务多到放不下时 tab 条横向滚动。状态由 DSH 自带的任务推送驱动，不轮询。
- **任务详情弹框**：每个 tab 上的 `ⓘ` 打开弹框（ESC / 点遮罩 / 关闭按钮都能关），里面是完整任务全文、模型、状态、开始 / 结束时间、耗时、费用、轮数、Claude 会话 id（可复制）和失败原因；缺省字段显示 `-`。点 `ⓘ` 不会切换选中的任务。
- **下方 Claude Code 窗口**：统计条（jobId / 状态 / 轮数 / 费用 / 耗时）+ **原生风格输出**——助手文本**直接按 Markdown 预览**（标题 / 粗体斜体 / 行内代码 / 代码块 / 有序无序列表 / 引用 / 分隔线 / 链接，每块右下角可切「原文 / 预览」；零依赖手写渲染器，全部走 React 文本节点、不用 `innerHTML`，链接只放行 `http(s):` 且 `target=_blank rel=noopener noreferrer`），思考块默认折叠成一行 `💭`（点开看全文），每次工具调用是一张卡片（工具名徽标按名字着色 + 参数 JSON 单行、点开展开为格式化全文），工具结果缩进挂在这张卡片下面（长结果截断 + 展开），任务结束是一条 `✅ 完成 · $0.13 · 12 turns · 3m20s` 摘要；贴底自动滚动、上滚即暂停并给「↓ 回到底部」。没有结构化事件的任务（例如首个块之前就失败）回落到等宽文本流。
- **操作**：取消（二次确认）、复制输出、复制 Claude 会话 id（可直接当 `resume` 用）。
- 实时输出只在「面板打开 + 选中任务还在跑」时每秒拉一次增量（结构化事件与文本流各自一个绝对游标），任务进终态后补拉一次收尾。

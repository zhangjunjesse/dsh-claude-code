...
- 内置 **Claude Code 监控面板**：会话头「对话 / 轨迹」右边多一个 **Claude Code** 标签页，顶部额度栏（5h/7d 进度条 + 模型专项 + 状态徽标）+ 任务列表 + 原生风格实时输出（工具卡片 / 可折叠思考）+ 一键取消（Web 端）。
...
- **顶部额度栏**：订阅档位（如 `Claude Max · 20x`）+ 5 小时 / 7 天两条窗口进度条（百分比、重置时间，≥50% 转黄、≥80% 转红）+ 模型专项徽标（如 `Fable 0%`）+ 正常 / 注意 / 已阻塞状态徽标 + 缓存年龄与刷新按钮。数据来自 claude CLI 自己的本地缓存（和 `claude_code_usage` 工具同源），刷新只是重读缓存、不会消耗额度；挂载时读一次，之后每 5 分钟一次。
- **左栏任务列表**：本会话的全部 `claude-code` 委派，运行中在前（按开始时间），已结束按新到旧；状态徽标 + 耗时实时走字。状态由 DSH 自带的任务推送驱动，不轮询。
- **右栏 Claude Code 窗口**：任务头（label / jobId / Claude 会话 id）+ 统计条（轮数 / 费用 / 耗时）+ **原生风格输出**——助手文本按正文排版，思考块默认折叠成一行 `💭`（点开看全文），每次工具调用是一张卡片（工具名徽标按名字着色 + 参数 JSON 单行、点开展开为格式化全文），工具结果缩进挂在这张卡片下面（长结果截断 + 展开），任务结束是一条 `✅ 完成 · $0.13 · 12 turns · 3m20s` 摘要；贴底自动滚动、上滚即暂停并给「↓ 回到底部」。没有结构化事件的任务（例如首个块之前就失败）回落到等宽文本流。

...
async function runClaude(task: string, opts: {
  cwd: string
  model: string
  permissionMode: string
  maxTurns: number
  ...
  for await (const msg of q) {
    if (msg.type === 'assistant') {
      ...
      for (const block of msg.message.content) {
        ...
        else if (block.type === 'tool_use') toolsUsed.add(block.name)
      }
      ...
      if (msg.subtype === 'success') {
        ...
        const detail = (msg as any).result ?? (msg as any).error ?? msg.subtype
        error = typeof detail === 'string' ? detail : JSON.stringify(detail)
      }
    }
  }

  if (error) throw new Error('claude_code failed: ' + error)
...
export function apply(ctx: Context, config: Config) {
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value: any) => [{ type: 'text', text: value.output }],
    },

...
async function runClaude(req, abort, onDelta) {
    let output = '';
    let sessionId = '';
    let costUsd = 0;
    let inputTokens = 0;
...
function startBackgroundJob(jobs, req, timeoutMs, owner) {
        ...
        run: () => {
            ...
            const done = (async () => {
                try {
                    ...
                    if (timedOut) {
                        return { status: 'failed', detail: `timed out after ${timeoutMs}ms`, output: full.snapshot() };
                    }
                    return { status: 'failed', detail: errorText(err), output: full.snapshot() };
                }
                finally {
                    if (timer)
                        clearTimeout(timer);

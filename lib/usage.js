...
const CLAUDE_MISSING_HINT = 'claude executable not found — install it with: npm install -g @anthropic-ai/claude-code';
...
function claudeAvailable(pathToClaudeCodeExecutable) {
    if (pathToClaudeCodeExecutable)
        return existsSync(pathToClaudeCodeExecutable);
...
function readAuthStatus(pathToClaudeCodeExecutable) {
...
function readExtra(raw, fallbackReason) {
    if (!isRecord(raw))
        return fallbackReason ? { isEnabled: false, disabledReason: fallbackReason } : null;
    return {
...
function deriveAdvice(windows, limits) {
    const percents = [];
 ...
 */
export function readUsageSnapshot(options = {}) {
...
export function renderUsage(snapshot) {
    ...
    const plan = [planType ? planType.charAt(0).toUpperCase() + planType.slice(1) : null, snapshot.subscription.rateLimitTier]
        ...
        .join(' / ');
    const peak = Math.max(snapshot.fiveHour?.utilizationPercent ?? 0, snapshot.sevenDay?.utilizationPercent ?? 0);
    const alarm = peak >= CAUTION_PERCENT || snapshot.advice === 'blocked' ? '⚠️ ' : '';
    const lines = [`${alarm}Claude 订阅额度${plan ? `（${plan}）` : ''}`];
    if (!snapshot.loggedIn)
        lines.push('· 登录态：已失效（下面的数字来自旧缓存）');
    ...
    if (sevenDay?.utilizationPercent !== null && sevenDay !== null) {
        ...
        lines.push(`· 7 天窗口：${sevenDay.utilizationPercent}%${reset ? `（${reset} 重置）` : ''}`
